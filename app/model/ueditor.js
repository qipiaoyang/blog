...
module.exports = class extends think.Service {
  ...
  spiderImage(imgUrl, filePath) {
    ...
    superagent.get(imgUrl) // 这里的URL也可以是绝对路径
  ...
  saveRemote() {
    ...
    return _asyncToGenerator(function* () {
      // think.log("dddddd")
      const imgUrl = _this.fileField;
      // imgUrl = imgUrl.replace(/&amp;/,"&");
      // http开头验证
      if (imgUrl.indexOf('http') !== 0) {
        _this.stateInfo = '链接不是http链接';
        return;
      }
      // TODO 各种验证后面弄
      ...
      _this.oriName = m || '';
      // console.log(this.oriName);
      _this.fileSize = imgUrl.length; // TODO 这里有问题，后面弄
      _this.fileType = _this.getFileExt;
      _this.fullName = _this.getFullName;
      _this.filePath = _this.getFilePath;
      _this.fileName = _this.getFileName;
      const filePath = _this.filePath;
      const fullName = _this.fullName;
      think.mkdir(_this.filePath.replace(_this.fileName, ''));
      const promises = yield _this.spiderImage(imgUrl, filePath);
      // console.log(promises);
      if (think.isFile(promises)) {
        _this.stateInfo = 'SUCCESS';
      } else {
        _this.stateInfo = '文件保存时出错';
      }
      return {
        'state': _this.stateInfo,
        'url': _this.fullName,
        'title': _this.fileName,
        'original': _this.oriName,
        'type': _this.fileType,
        'size': _this.fileSize
      };
    })();
