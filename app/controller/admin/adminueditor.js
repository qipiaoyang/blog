...
module.exports = class extends think.Controller {
  ...
  islogin() {
    ...
    return _asyncToGenerator(function* () {
      // 判断是否登录
      const user = yield _this2.session('userInfo');
  ...
  indexAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const action = _this3.get('action');
      // think.log(action);
      let result;
      switch (action) {
        case 'config':
          result = _this3.config;
        ...
        case 'uploadfile':
          ...
          result = yield _this3.uploads();
          // console.log(result);
          break;

        /* 列出图片 */
        case 'listimage':
          result = _this3.uploadlist();
          break;
        /* 列出文件 */
        case 'listfile':
          result = _this3.uploadlist();
  ...
  uploads() {
    ...
    return _asyncToGenerator(function* () {
       ...
       * }
       */
      const action = _this4.get('action');
      let base64 = 'upload';
      let config = {};
      let fieldName;
      // console.log(setup);
      switch (action) {
        case 'uploadimage':
          config = {
            pathFormat: _this4.config['imagePathFormat'],
            maxSize: _this4.config['imageMaxSize'],
            allowFiles: _this4.config['imageAllowFiles']
          };
          fieldName = _this4.config['imageFieldName'];
          break;
        case 'uploadscrawl':
          config = {
            'pathFormat': _this4.config['scrawlPathFormat'],
            'maxSize': _this4.config['scrawlMaxSize'],
            'allowFiles': _this4.config['scrawlAllowFiles'],
            'oriName': 'scrawl.png'
          };
          fieldName = _this4.config['scrawlFieldName'];
          base64 = 'base64';
          break;
        case 'uploadvideo':
          config = {
            'pathFormat': _this4.config['videoPathFormat'],
            'maxSize': _this4.config['videoMaxSize'],
            'allowFiles': _this4.config['videoAllowFiles']
          };
          fieldName = _this4.config['videoFieldName'];
          break;
        case 'uploadfile':
        default:
          config = {
            'pathFormat': _this4.config['filePathFormat'],
            'maxSize': _this4.config['fileMaxSize'],
            'allowFiles': _this4.config['fileAllowFiles']
          };
          fieldName = _this4.config['fileFieldName'];
          break;
      }
      // 加入七牛接口
      if (Number(think.config('ext.qiniu.is')) === 1 && base64 === 'upload') {
        const file = think.extend({}, _this4.file(fieldName));
        // console.log(file);
        const filepath = file.path;
        const extname = path.extname(file.name);
        const basename = path.basename(filepath) + extname;
        const qiniu = _this4.extService('qiniu', 'qiniu');
        const uppic = yield qiniu.uploadpic(filepath, basename);
        ...
        const up = _this4.extService('ueditor', 'editor', fieldName, config, base64, _this4.ctx); // 加载名为 ueditor 的 editor Adapter
  ...
  crawler() {
    ...
    return _asyncToGenerator(function* () {
      /* 上传配置 */
      const config = {
        'pathFormat': _this5.config['catcherPathFormat'],
        'maxSize': _this5.config['catcherMaxSize'],
        'allowFiles': _this5.config['catcherAllowFiles'],
        'oriName': 'remote.png'
      };
      const fieldName = _this5.config['catcherFieldName'];
      let source = _this5.post(fieldName + '[]');
      ...
      for (const imgUrl of source) {
        const up = _this5.extService('ueditor', 'editor', imgUrl, config, 'remote'); // 加载名为 ueditor 的 editor Adapter
        const info = yield up.saveRemote();
        // console.log(info);
...
//# sourceMappingURL=adminueditor.js.map